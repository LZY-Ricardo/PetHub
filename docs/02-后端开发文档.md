...
CREATE TABLE `pet_info` (
  ...
  `status` ENUM('available', 'pending', 'adopted', 'off_shelf') DEFAULT 'available' COMMENT '状态',
...
- source_type：发布来源，platform（平台发布）、user（用户发布）
- submission_status：发布审核状态，pending（待审核）、approved（已通过）、rejected（已驳回）
- submission_comment：发布审核备注
- status：宠物状态，available（待领养）、pending（审核中/申请中）、adopted（已领养）、off_shelf（已下架）
- photos：照片URL数组，JSON格式
- remarks：其他备注信息
- created_by：创建人ID（管理员）
- owner_user_id：送养发布用户ID，平台发布时可为空
- adopted_by：领养人ID
- adopted_at：领养时间
- created_at：创建时间
- updated_at：更新时间
