...
class ForumService {
  ...
  async deletePost(id, viewer) {
    const post = await ForumDAO.getPostDetail(id);
    if (!post) {
      throw new Error('帖子不存在');
    }

    const isOwner = Number(post.user_id) === Number(viewer?.userId);
  ...
  async updatePostCategory(id, viewer, category) {
    const allowedCategories = ['经验分享', '求助问答', '宠物展示', '闲聊灌水'];
  ...
  async deleteComment(id, viewer) {
    ...
    const isOwner = Number(comment.user_id) === Number(viewer?.userId);
    const isAdmin = viewer?.role === 'admin';
